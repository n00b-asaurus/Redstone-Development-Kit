...
class SingleOff(BuildHandler):
	def __init__(self):
		pass
	
	def type(self):
		return "single off connection build handler"
	...
	def build_structure(self, commands, x, z):
		...
		commands.append(setblock(Position(x+0,2,z+0), repeater('+x')))
		commands.append(setblock(Position(x+1,2,z+0), base_block()))
		commands.append(fill(Position(x+0,3,z+0), Position(x+1,3,z+0), base_block()))
		commands.append(fill(Position(x+0,4,z+0), Position(x+1,4,z+0), redstone_dust()))
		
	def place_input_repeater(self, commands, x, z):
		commands.append(fill(Position(x+0,3,z+0), Position(x+1,4,z+0), air()))
