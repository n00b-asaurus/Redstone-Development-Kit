from structure_generator.constructors.properinglish19_decoder.build_handler import BuildHandler
...
class RightDoubleOff(BuildHandler):
	def __init__(self):
		pass
	
	def type(self):
		return "right double off build handler"
	...
	def build_structure(self, commands, x, z):
		...
		commands.append(setblock(Position(x+0,2,z+0), redstone_dust()))
		commands.append(setblock(Position(x+1,2,z+0), base_block()))
		commands.append(setblock(Position(x+1,3,z+0), redstone_dust()))
		
		commands.append(setblock(Position(x+0,0,z-1), base_block()))
		commands.append(setblock(Position(x+0,1,z-1), repeater('-z')))
		commands.append(setblock(Position(x+0,1,z-2), base_block()))
		
	def place_input_repeater(self, commands, x, z):
		commands.append(setblock(Position(x+0,3,z+0), base_block()))
		commands.append(setblock(Position(x-1,3,z+0), repeater('+x')))
